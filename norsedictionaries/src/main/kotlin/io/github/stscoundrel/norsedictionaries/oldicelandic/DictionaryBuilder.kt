...
private const val jsonPath = "old-icelandic-dictionary.json"
private const val databasePath = "src/main/resources/old-icelandic-dictionary.db"


fun createOldIcelandicDatabase() {
...
private fun getDictionaryEntries(): List<OldIcelandicDictionaryEntry> {
    val jsonContent = readResourceFile(jsonPath)
    return Json.decodeFromString<List<OldIcelandicDictionaryEntry>>(jsonContent)
}

private fun createDatabaseTable(connection: Connection) {
    val statement = connection.createStatement()
    val sql = """
        CREATE TABLE IF NOT EXISTS old_icelandic (
...
private fun createDatabaseEntries(connection: Connection, entries: List<OldIcelandicDictionaryEntry>) {
    val sql = "INSERT INTO old_icelandic (headword, definitions) VALUES (?, ?)"
